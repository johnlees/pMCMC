...
int get_tree(char *filename,TREE *t) {
	...
	char separator[20];
	if((in=fopen("geneTree/sim.tree","r"))==NULL) {
		printf("\n\tCannot open %s\n",filename);
		exit(1);
	}
...
double replicate(TREE *t, TREE *s, double theta, double m, long rep) {
	...
	xrep=(double)rep;
	//s=(TREE*)c_malloc(sizeof(TREE));
	//get_tree(tree_name,s); //Why does it load in the tree multiple times?
	for(i=0;i<=n_sites;i++) {
...
// 	theta=2.5;
//	rep=100;
//	strcpy(tree_name,"sim.tree"); /* We at least need a tree */
//	
//	
//	xrep=(double)rep;
...
// 	double prob = replicate(t,theta,1,rep);
// 	printf("\n\tEstimated likelihood of the tree %.36e",prob);
//	return 0;
//}
